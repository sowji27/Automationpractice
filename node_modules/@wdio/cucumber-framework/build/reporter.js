...
class CucumberReporter {
    constructor(eventBroadcaster, options, cid, specs, reporter) {
        ...
        this.eventListener = new cucumberEventListener_1.default(eventBroadcaster)
    ...
    handleBeforeFeature(uri, feature) {
        ...
        this.emit('suite:start', {
            uid: utils_1.getUniqueIdentifier(feature),
    ...
    handleBeforeScenario(uri, feature, scenario) {
        ...
        this.emit(this.scenarioLevelReport ? 'test:start' : 'suite:start', {
            uid: utils_1.getUniqueIdentifier(scenario),
            title: this.getTitle(scenario),
            parent: utils_1.getUniqueIdentifier(feature),
            type: 'scenario',
            file: uri,
            tags: scenario.tags,
    ...
    handleBeforeStep(uri, feature, scenario, step) {
        this.testStart = new Date();
        const type = utils_1.getStepType(step.type);
    ...
    afterTest(uri, feature, scenario, step, result, sourceLocation = null) {
        ...
        let title = step ? utils_1.getTestStepTitle(step.keyword, step.text) : this.getTitle(scenario);
        ...
        else if (result.status === cucumber_1.Status.FAILED) {
            if (!result.retried) {
                this.failedCount++;
            }
        ...
        else if (result.status === cucumber_1.Status.AMBIGUOUS && this.options.failAmbiguousDefinitions) {
            state = 'fail';
            this.failedCount++;
            error = {
                message: result.exception,
        ...
        const payload = step ? utils_1.buildStepPayload(uri, feature, scenario, step, {
            type: 'step',
            ...common
        }) : {
            type: 'scenario',
            uid: utils_1.getUniqueIdentifier(scenario, sourceLocation),
            parent: utils_1.getUniqueIdentifier(feature),
            tags: scenario.tags,
            ...common
        };
    ...
    getTitle(featureOrScenario) {
        ...
        if (!this.tagsInTitle || !tags.length)
            return name;
